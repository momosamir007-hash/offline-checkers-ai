...
"""
╔══════════════════════════════════════════════════════╗
║       ♟️ مساعد الداما الذكي — النسخة النهائية v9     ║
║  Bitboard Engine + Groq LLM Coach + OpenCV Vision    ║
║  عمق 15-22 + شرح ذكي + تحليل صور + تعديل يدوي      ║
╚══════════════════════════════════════════════════════╝
"""
...
for _sq in range(32):
    _r, _c = SQ_TO_RC[_sq]
    # الأبيض: كلما اقترب من الصف 0 (ترقية) كان أفضل
    w_val = (7 - _r) * 3
    if _r == 7:
        w_val = 5  # صف خلفي = حماية
    if _r <= 1:
        w_val += 15  # قريب جداً من الترقية
    if 2 <= _r <= 5 and 2 <= _c <= 5:
        w_val += 4  # مركز
    W_POS_VAL.append(w_val)

    # الأسود: كلما اقترب من الصف 7 (ترقية) كان أفضل
    b_val = _r * 3
    if _r == 0:
        b_val = 5
    if _r >= 6:
...
class BB:
    ...
    def _find_jumps(self, sq, dirs, my, opp, empty,
        ...
        for d in dirs:
            ...
            if mid in eaten:
                continue
            if not ((1 << land) & empty):
                # السماح بالهبوط على مربع البداية الأصلي
                if land != path[0]:
                    continue
            ...
            new_path = path + [land]
            new_eaten = eaten | {mid}
            # تحديث empty و opp للأكل المتعدد
    ...
    def full_winner(self):
        """فحص كامل بما في ذلك عدم وجود حركات"""
        w = self.winner()
        if w: return w
    ...
    def move_to_rc(self, move):
        return tuple(SQ_TO_RC[s] for s in move)
...
class Beast:
    ...
    def evaluate(self, bb, for_white):
        ...
        for sq in iter_bits(bb.wp):
            ...
            # 8. هل أنا مهدد بالأكل؟
            for d in range(4):
                mid, land = JUMP_TABLE[sq][d]
                if mid != -1 and land != -1:
                    # هل خصم على mid يستطيع القفز فوقي؟
                    opp_d = d ^ 2  # الاتجاه المعاكس (0↔2, 1↔3)
                    attacker_sq = NBR[sq][opp_d] if opp_d < 4 else -1
                    # فعلياً: هل خصم يجلس بحيث يقفز فوقي؟
                    # الخصم على attacker_sq يقفز في الاتجاه d ليأكلني
                    if attacker_sq == -1:
                        continue
                    for ad in range(4):
            ...
            # 9. قطعة هاربة (clear path to promotion)
            if not is_k:
                r = SQ_TO_RC[sq][0]
                clear = True
                for rr in range(r):
            ...
            if is_k:
                ...
                if r in (0, 7) or c in (0, 7):
                    ...
                    for d in range(4):
                        nb = NBR[sq][d]
                        if nb != -1 and not ((1 << nb) & occ):
                            exits += 1
                    if exits <= 1:
                        sc -= 8

        # نفس الأنماط للأسود (معكوسة)
    ...
    def _search(self, bb, depth, alpha, beta, white, for_w, pv=True):
        ...
        for mv in moves:
            ...
            if i == 0:
                sc = -self._search(child, depth - 1, -beta, -alpha,
                                   not white, for_w, True)[0]
            else:
                sc = -self._search(child, depth - 1 - red, -alpha - 1, -alpha,
                                   not white, for_w, False)[0]
                if alpha < sc < beta and not self.stop:
                    if red > 0:
                        sc = -self._search(child, depth - 1, -alpha - 1, -alpha,
                                           not white, for_w, False)[0]
                    if alpha < sc < beta and not self.stop:
                        sc = -self._search(child, depth - 1, -beta, -sc,
                                           not white, for_w, True)[0]
    ...
    def find_best(self, bb, white):
        ...
        for d in range(1, 50):
            sc, mv = self._search(bb, d, float("-inf"), float("inf"),
                                  white, white, True)
            if self.stop: break
            if mv is not None:
...
استخدم إيموجي وكن حماسياً."""
    ...
    def _basic(self, best, moves, ev, white):
        ...
        if best["is_capture"]:
            lines.append("💥 **السبب:** تأكل قطعة للخصم!")
        elif best["promotes"]:
            lines.append("👑 **السبب:** ترقية للملك!")
        elif best["score"] > 100:
            lines.append("💪 **السبب:** تعزز تفوقك")
        elif best["score"] > 0:
            lines.append("✅ **السبب:** تحسّن وضعيتك")
        else:
            lines.append("🛡️ **السبب:** أفضل دفاع متاح")
    ...
    def ask(self, grid, question, white):
        if not self.ok:
            return "⚠️ أضف مفتاح Groq لتفعيل المدرب!\n🔑 https://console.groq.com"
        color = "الأبيض" if white else "الأسود"
        board_txt = self._grid_text(grid)
...
class Vision:
    ...
    def process(img_bgr, lt=160, dt=100):
        """خط أنابيب كامل: تصحيح → HSV → دوائر → دمج"""
        ...
        for r in range(8):
            for c in range(8):
                ...
                if h != 0 and ci != 0 and h == ci:
                    merged[r][c] = h  # متفقين = ثقة عالية
                elif ci != 0:
                    merged[r][c] = ci  # الدوائر أدق
                elif h != 0:
                    merged[r][c] = h
        ...
        return {
            "merged": merged,
            "hsv_board": board_hsv,
            "circle_board": board_circles,
            "fixed_pil": fixed_pil,
            "circles_vis": circles_vis,
            "was_fixed": was_fixed,
            "hsv_info": hsv_info,
        }
    ...
    def _detect_circles(img, lt, dt):
        ...
        c = 50
        circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1.2, 35,
                                    param1=60, param2=35,
        ...
        if circles is not None:
            for cx, cy, rad in np.uint16(np.around(circles))[0]:
                ...
                a = float(np.mean(s))
                if a > lt:
                    board[ro][co] = 1; clr = (0, 255, 0)
                elif a < dt:
                    board[ro][co] = 2; clr = (0, 0, 255)
                else:
                    continue
...
class Ren:
    ...
    def draw(grid, arrows=None, hl=None):
        ...
        if arrows:
            for a in arrows:
                Ren._arrow(dr, a["m"], a.get("c", (255, 50, 50)), a.get("w", 5))
        return img
...
def app():
    ...
    if "coach" not in st.session_state:
        st.session_state.coach = DamaCoach()

    st.title("♟️ مساعد الداما الذكي")
    st.caption("Bitboard Engine + Groq AI Coach + OpenCV Vision")
    ...
    with st.sidebar:
        ...
        if api_key:
            st.session_state.coach = DamaCoach(api_key)
            st.success("✅ المدرب مفعّل!")
        else:
            st.session_state.coach = DamaCoach()
            st.caption("💡 أضف مفتاح Groq المجاني لشرح ذكي")
    ...
    # ─── تبويب: إدخال يدوي ───
    with tabs[0]:
        st.markdown("**اختر قطعة واضغط على المربع لوضعها:**")
    ...
    if HAS_CV2:
        with tabs[1]:
            ...
            st.markdown("ارفع صورة → يكتشف القطع تلقائياً → عدّل يدوياً إذا أردت → حلّل")
            ...
            if up:
                ...
                st.markdown("**⚙️ ضبط حساسية الكشف:**")
                ...
                if st.button("🔍 تحليل الصورة", type="primary"):
                    ...
                    with c2:
                        st.image(result["fixed_pil"],
                                 caption="✅ مصحح المنظور" if result["was_fixed"] else "📐 مقتصة",
                                 use_container_width=True)

                    st.success("✅ اكتمل التحليل!")

                    # عرض النتائج في تبويبات
                    rt1, rt2, rt3 = st.tabs(["🔀 مدمج", "🎨 HSV", "⭕ دوائر"])

                    with rt1:
                        st.image(Ren.draw(result["merged"]),
                                 caption="النتيجة المدمجة", use_container_width=True)
                        bb_tmp = BB.from_grid(result["merged"])
                        wn_t = popcount(bb_tmp.wp)
                        bn_t = popcount(bb_tmp.bp)
                        st.info(f"⚪ أبيض: {wn_t} قطعة  •  ⚫ أسود: {bn_t} قطعة")

                    with rt2:
                        st.image(Ren.draw(result["hsv_board"]),
                                 caption="نتيجة HSV", use_container_width=True)
                        with st.expander("🔬 تفاصيل HSV"):
                            for d in result["hsv_info"]:
                                if d["d"] != 0:
                                    nm = "⚪" if d["d"] == 1 else "⚫"
                                    st.text(f"({d['r']},{d['c']}) {nm} "
                                            f"سطوع={d['br']} تشبع={d['sa']} تباين={d['va']}")

                    with rt3:
                        st.image(result["circles_vis"],
                                 caption="الدوائر المكتشفة", use_container_width=True)
                        st.image(Ren.draw(result["circle_board"]),
                                 caption="نتيجة الدوائر", use_container_width=True)

                    # ── زر الاستيراد + التعديل ──
                    st.markdown("---")
                    st.markdown("### ✏️ استيراد وتعديل")
                    st.markdown("اضغط **استيراد** ثم عدّل القطع الخاطئة في تبويب **الإدخال اليدوي**")

                    if st.button("📥 استيراد للرقعة (يمكنك التعديل بعدها)", type="primary"):
                        st.session_state.board = result["merged"].tolist()
                        st.success("✅ تم الاستيراد! اذهب لتبويب ✏️ لتعديل أو 🧠 للتحليل")
                        st.rerun()
    ...
    with tabs[ai_idx]:
        ...
        if st.button("🧠 حلّل الآن!", type="primary", use_container_width=True):
            prg = st.empty()
            prg.info(f"🧠 Bitboard Engine يحلل ({tt}s)...")
            ...
            st.markdown(f'<div class="ebar"><div class="efill" '
                        ...
                        f'{em} ({pe})</div></div>', unsafe_allow_html=True)

            # ── أكل إجباري ──
            if analysis["forced"]:
                st.markdown('<div class="warn">⚡ أكل إجباري! يجب عليك الأكل</div>',
            ...
            with col1:
                bi = Ren.draw(ba2,
                              arrows=[{"m": list(rc), "c": (50, 205, 50), "w": 6}],
                              hl=set(rc))
                st.image(bi, caption="🏆 أفضل حركة", use_container_width=True)
            with col2:
                af = bb.copy()
                af.do_move(best["move"], fw)
                st.image(Ren.draw(af.to_grid()),
                         caption="📋 الرقعة بعد التنفيذ", use_container_width=True)

            # ── زر التطبيق ──
            if st.button("✅ طبّق هذه الحركة على الرقعة", use_container_width=True):
                st.session_state.board = af.to_grid().tolist()
                st.rerun()
            ...
            if coach.ok:
                ...
                question = st.text_input("اكتب سؤالك عن الوضعية:",
                                          placeholder="مثال: لماذا لا أحرك القطعة اليسرى؟")
                if question:
            ...
            # ── إحصائيات الأداء ──
            st.markdown(f"""<div class="perf">
                ⚡ <b>Bitboard Engine:</b>
                عمق {best['depth']} •
                {analysis.get('tt_hits', 0):,} TT hits •
            ...
            rc_colors = {1: ("🥇", (50, 205, 50)), 2: ("🥈", (65, 105, 225)),
                         3: ("🥉", (255, 165, 0))}
            ...
            for i, mv in enumerate(mvs[:5]):
                ...
                top_arrows.append({"m": list(mv["rc"]), "c": cl,
                                   "w": 6 if i == 0 else 3})
            st.image(Ren.draw(ba2, arrows=top_arrows),
                     caption="🥇 أخضر = الأفضل  🥈 أزرق = ثاني  🥉 برتقالي = ثالث",
                     use_container_width=True)
            ...
            for mv in mvs:
                ...
                with st.expander(f"{icon} {p} • {mv['score']} • {mv['win_pct']}% • {mv['verdict']}"):
                    st.markdown(f"**المسار:** `{p}`")
                    st.markdown(f"**الحكم:** {mv['verdict']}")
                    st.markdown(f"**فرصة الفوز:** `{bar}` {mv['win_pct']}%")
                    st.markdown(f"**عمق التحليل:** {mv['depth']}")

                    mi = Ren.draw(ba2,
                                  arrows=[{"m": list(mv["rc"]), "c": (255, 100, 50), "w": 5}],
                                  hl=set(mv["rc"]))
    ...
    st.markdown(
        ...
        '♟️ v9.0 — Bitboard + Zobrist TT + PVS + LMR + Quiescence + '
        'Groq LLM Coach + OpenCV Vision — '
        'يعمل بدون إنترنت (بدون شرح LLM فقط)</p>',
        unsafe_allow_html=True
    )
