...
"""
╔══════════════════════════════════════════════════════╗
║       ♟️ مساعد الداما الذكي — النسخة الاستراتيجية v15  ║
...
import time, random, json, urllib.request, urllib.error, os, math
...
if HAS_XGB and MODEL_PATH.exists():
    try:
        ...
        # The bundled model is auxiliary only. Record metadata so incompatible
        # replacements fail closed instead of corrupting the search evaluation.
        _cfg = json.loads(AI_MODEL.save_config())
        _obj = _cfg.get("learner", {}).get("objective", {}).get("name", "")
        _nf = int(AI_MODEL.num_features())
        if _nf != 65 or _obj not in {"reg:squarederror", "reg:absoluteerror"}:
            print(f"⚠️ نموذج غير متوافق: features={_nf}, objective={_obj}")
            AI_MODEL = None
...
# ══════════════════════════════════════════════════════
# 2.1 أدوات سلامة وتحليل الوضعية
# ══════════════════════════════════════════════════════

def validate_grid(grid):
    """Validate a user/image supplied 8x8 position without silently fixing it."""
    try:
        a = np.asarray(grid, dtype=np.int16)
    except Exception:
        return False, ["تعذر قراءة بيانات الرقعة"]
    errors = []
    if a.shape != (8, 8):
        return False, ["يجب أن تكون الرقعة 8×8"]
    if np.any((a < 0) | (a > 4)):
        errors.append("توجد قيم قطع غير معروفة")
    for r in range(8):
        for c in range(8):
            if (r + c) % 2 == 0 and int(a[r, c]) != 0:
                errors.append(f"قطعة على مربع فاتح ({r},{c})")
    white = int(np.sum((a == 1) | (a == 3)))
    black = int(np.sum((a == 2) | (a == 4)))
    if white > 12 or black > 12:
        errors.append("عدد القطع يتجاوز الحد القانوني (12)")
    if white == 0 and black == 0:
        errors.append("الرقعة فارغة")
    return not errors, errors


def move_notation(move):
    """Stable 1..32 notation; x denotes capture and - denotes a quiet move."""
    if not move:
        return "—"
    sep = 'x' if len(move) > 2 or (len(move) == 2 and abs(SQ_TO_RC[move[0]][0] - SQ_TO_RC[move[1]][0]) == 2) else '-'
    return sep.join(str(int(x) + 1) for x in move)


def _capture_count(bb, move):
    """Count actual captured pieces, independent of the path length."""
    n = 0
    for s, e in zip(move, move[1:]):
        sr, sc = SQ_TO_RC[s]; er, ec = SQ_TO_RC[e]
...
# ══════════════════════════════════════════════════════
# 2.2 التحليل الاستراتيجي وتسجيل المباراة — v15
# ══════════════════════════════════════════════════════

def position_fingerprint(bb, white_turn):
    """Stable repetition key that does not depend on the transposition-table depth."""
    return (bb.wp, bb.bp, bb.k, bool(white_turn))

def material_summary(bb):
    wm = popcount(bb.wp & ~bb.k); wk = popcount(bb.wp & bb.k)
    bm = popcount(bb.bp & ~bb.k); bk = popcount(bb.bp & bb.k)
    return {"white_men": wm, "white_kings": wk, "black_men": bm, "black_kings": bk,
            "white_total": wm + wk, "black_total": bm + bk}

def strategic_profile(bb, white_turn):
    """Explain a position with transparent, rule-level strategic signals."""
    wmoves, wcap = bb.get_moves(True)
    bmoves, bcap = bb.get_moves(False)
    occ = bb.wp | bb.bp

    def advanced(side_white):
        pieces = (bb.wp if side_white else bb.bp) & ~bb.k
        score = 0
        for sq in iter_bits(pieces):
            r, _ = SQ_TO_RC[sq]
            score += (7-r) if side_white else r
        return score

    def threatened(side_white):
        mine = bb.wp if side_white else bb.bp
        opp = bb.bp if side_white else bb.wp
        count = 0
        for sq in iter_bits(mine):
            for d in K_DIRS:
                mid, land = JUMP_TABLE[sq][d]
                # Opponent jumps over sq from the opposite side.
                if mid == -1 or land == -1:
                    continue
                sr, sc = SQ_TO_RC[sq]
                lr, lc = SQ_TO_RC[land]
                ar, ac = 2*sr-lr, 2*sc-lc
                attacker = RC_TO_SQ.get((ar, ac))
                if attacker is not None and (opp & (1 << attacker)) and not (occ & (1 << land)):
                    count += 1
                    break
        return count

    wth = threatened(True); bth = threatened(False)
    return {
        "material": material_summary(bb),
        "white_mobility": len(wmoves), "black_mobility": len(bmoves),
        "white_capture": bool(wcap), "black_capture": bool(bcap),
        "white_advanced": advanced(True), "black_advanced": advanced(False),
        "white_threatened": wth, "black_threatened": bth,
        "side_to_move": "الأبيض" if white_turn else "الأسود",
        "phase": "نهاية اللعب" if (popcount(bb.wp)+popcount(bb.bp)) <= 8 else
                 ("منتصف اللعب" if (popcount(bb.wp)+popcount(bb.bp)) <= 16 else "الافتتاح/بداية المباراة")
    }

class GameTracker:
    """Tracks a game, repetitions and irreversible captures/promotions."""
    def __init__(self, bb=None, white_turn=True):
        self.reset(bb or BB(), white_turn)

    def reset(self, bb, white_turn=True):
        self.start = bb.copy()
        self.position = bb.copy()
    ...
    def _record_position(self):
        key = position_fingerprint(self.position, self.white_turn)
        self.repetitions[key] = self.repetitions.get(key, 0) + 1

    def legal_moves(self):
        return self.position.get_moves(self.white_turn)[0]

    def play(self, move):
        legal = self.legal_moves()
        move = tuple(move)
        if move not in legal:
            raise ValueError("حركة غير قانونية في الوضعية الحالية")
        before_k = self.position.k
        captured = _capture_count(self.position, move)
        start = move[0]; end = move[-1]
        was_king = bool(before_k & (1 << start))
        self.position.do_move(move, self.white_turn)
        promoted = (not was_king) and bool(self.position.k & (1 << end))
        self.history.append({
            "ply": len(self.history)+1,
            "side": "white" if self.white_turn else "black",
            "move": move,
    ...
    def undo(self):
        if not self.history:
            return False
        # Rebuild from the initial position; this is robust and avoids hidden mutation bugs.
        hist = self.history[:-1]
        self.position = self.start.copy()
        self.white_turn = True if not self.history else (self.history[0]["side"] == "black")
        self.history = []
        self.repetitions = {}
        self._record_position()
        for item in hist:
    ...
    def repetition_count(self):
        return self.repetitions.get(position_fingerprint(self.position, self.white_turn), 0)

    def status(self):
        winner = self.position.status(self.white_turn)
        if winner is not None:
            return {"state": "win", "winner": winner}
        if self.repetition_count() >= 3:
            return {"state": "draw", "reason": "threefold_repetition"}
        return {"state": "active"}

def analyze_game_history(tracker, engine=None, max_time_per_move=0.35):
    """Re-evaluate each recorded move and identify turning points."""
    if not tracker.history:
        return {"moves": [], "turning_points": [], "summary": "لا توجد نقلات مسجلة بعد."}

    pos = tracker.start.copy()
    side = tracker.history[0]["side"] == "white"
    rows, turning = [], []
    prev_eval = None

    for item in tracker.history:
        legal, _ = pos.get_moves(side)
        if item["move"] not in legal:
            break
        local = engine or Beast(max_time=max_time_per_move)
        if engine is None:
            local.max_time = max_time_per_move
        before = local.evaluate(pos, side)
        alternatives = local.analyze_all(pos, side)
        best_score = alternatives["moves"][0]["score"] if alternatives else None
        chosen = next((x for x in (alternatives or {}).get("moves", []) if x["move"] == item["move"]), None)

        pos.do_move(item["move"], side)
        after = local.evaluate(pos, not side)
        mover_view_after = -after
        loss = max(0.0, best_score - mover_view_after) if best_score is not None else 0.0
        label = "أفضل نقلة" if loss < 20 else ("عدم دقة" if loss < 60 else ("خطأ" if loss < 140 else "خطأ جسيم"))
        row = {
            "ply": item["ply"], "side": item["side"], "notation": item["notation"],
            "eval_before": round(before, 1), "eval_after": round(mover_view_after, 1),
            "score_loss": round(loss, 1), "label": label,
            "best_notation": alternatives["best_notation"] if alternatives else item["notation"],
            "chosen_rank": chosen["rank"] if chosen else None
        }
        rows.append(row)
        if prev_eval is not None and abs(mover_view_after - prev_eval) >= 120:
            turning.append(row)
        prev_eval = mover_view_after
        side = not side

    return {
        "moves": rows,
        "turning_points": turning,
        "summary": f"تم تحليل {len(rows)} نقلة، وتم العثور على {len(turning)} نقطة تحول محتملة."
    }
...
class Beast:
    ...
    def evaluate(self, bb, white_turn):
        ...
        if ml_score is not None:
            sc = 0.90 * sc + 0.10 * ml_score
        # Mobility and forcing pressure add strategic information that material
        # alone cannot see. They are intentionally small to avoid tactical noise.
        wmoves, wcap = bb.get_moves(True)
        bmoves, bcap = bb.get_moves(False)
        sc += (len(wmoves) - len(bmoves)) * (2 if endgame else 1)
        if wcap: sc += 6
        if bcap: sc -= 6
    ...
    def _quiesce(self, bb, alpha, beta, white, qd=0):
        ...
        for idx, mv in enumerate(moves):
            remaining = (self.deadline if self.deadline is not None else time.time()) - time.time()
            if remaining <= 0:
                break
    ...
    def principal_variation(self, bb, white, first_move=None, max_len=12):
        """Extract a legal principal variation from the current TT safely."""
        line, pos, side = [], bb.copy(), white
        mv = first_move
        seen = set()
        for _ in range(max_len):
            key_state = (pos.wp, pos.bp, pos.k, side)
            if key_state in seen: break
            seen.add(key_state)
            if mv is None: mv = self.tt.best(pos.zobrist(side))
            legal, _ = pos.get_moves(side)
            if mv is None or mv not in legal: break
            line.append(tuple(mv))
            pos.do_move(mv, side)
            side = not side
            if pos.status(side) is not None: break
            mv = None
        return line

    def find_best(self, bb, white):
        self.nodes = 0; self.t0 = time.time()
        self.deadline = self.t0 + self.max_time
    ...
    def analyze_all(self, bb, white):
        """Analyze every legal move under one strict total time budget."""
        self.tt.clear(); self._eval_cache = {}
        self.nodes = 0; self.t0 = time.time(); self.deadline = self.t0 + self.max_time
        moves, is_cap = bb.get_moves(white)
        ...
        for idx, mv in enumerate(moves):
            ...
            # Reserve a fair share while allowing the final moves to use leftover time.
            per_move = max(0.04, remaining / max(1, len(moves) - idx))
            child = bb.copy(); child.do_move(mv, white)
            sub = Beast(max_time=per_move)
            sub.tt = self.tt
            res = sub.find_best(child, not white)
            sc = -res['score']
            reply = res.get('move')
            is_capt = len(mv) > 2 or (len(mv) == 2 and abs(SQ_TO_RC[mv[0]][0] - SQ_TO_RC[mv[1]][0]) == 2)
            cap_n = _capture_count(bb, mv)
            end = mv[-1]; piece_bit = 1 << mv[0]
            promo = ((white and (1 << end) & W_PROMO) or (not white and (1 << end) & B_PROMO)) and not (piece_bit & bb.k)
            if is_capt and cap_n >= 2: verdict = '🔥 أكل متعدد!'
            elif promo: verdict = '👑 ترقية!'
            elif sc > 300: verdict = '💪 ساحقة'
            elif sc > 100: verdict = '✅ ممتازة'
            elif sc > 30: verdict = '✅ جيدة'
            elif sc > -30: verdict = '⚖️ متكافئة'
            elif sc > -100: verdict = '⚠️ حذر'
            else: verdict = '❌ تجنبها'
            results.append({'move': mv, 'rc': tuple(SQ_TO_RC[x] for x in mv), 'score': round(sc,1),
                'depth': res['depth'], 'is_capture': is_capt, 'captured': cap_n, 'promotes': promo,
                'verdict': verdict, 'notation': move_notation(mv), 'win_pct': int(round(100.0 / (1.0 + math.exp(-max(-800.0, min(800.0, sc)) / 180.0)))),
                'reply': reply, 'reply_rc': tuple(SQ_TO_RC[x] for x in reply) if reply else ()})
        if not results: return None
        results.sort(key=lambda x:x['score'], reverse=True)
        for i,r in enumerate(results):
            r['rank']=i+1; loss=max(0.0,results[0]['score']-r['score']); r['score_loss']=round(loss,1)
            r['mistake_level']='الأفضل' if i==0 else ('طفيف' if loss<30 else 'متوسط' if loss<100 else 'كبير')
            r['tactical_note'] = ('سلسلة أكل متعددة' if r['captured'] >= 2 else 'يفرض رداً مباشراً' if r['is_capture'] else 'ترقية محتملة' if r['promotes'] else 'يحسن التموضع')
        return {'moves':results,'time':round(time.time()-self.t0,2),'forced':is_cap,
                'eval':round(self.evaluate(bb,white),1),'tt_hits':self.tt.hits,
                'best_notation': results[0]['notation'],
                'coverage':f"{len(results)}/{len(moves)}",
                'strategic_profile': strategic_profile(bb, white)}
...
def app():
    ...
    st.title("♟️ مساعد الداما الذكي v14 (Professional Engine)")
    st.caption("محرك بحث تكتيكي + تقييم هجين + تحليل أخطاء + مدرب اختياري")
    ...
    with st.sidebar:
        ...
        st.divider()
        st.markdown("### 🧠 نموذج التقييم")
        st.caption("النموذج يُستخدم كمُصحّح مساعد محدود التأثير، بينما يبقى المحرك الكلاسيكي المسؤول الرئيسي عن القرار.")
